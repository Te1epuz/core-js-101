...
function timeSpanToString(startDate, endDate) {
  ...
  const hours = Math.floor(date / 1000 / 60 / 60).toString().padStart(2, '0');
  ...
  const milliSecondes = (date % 1000).toString().padStart(3, '0');
...
function angleBetweenClockHands(date) {
  const hoursHand = (date.getUTCHours() % 12) * 30 + date.getUTCMinutes() / 2;
  const minutesHand = date.getUTCMinutes() * 6;
  let diff = hoursHand - minutesHand;
  if (diff > 180) diff = 360 - diff;
  const result = (diff / 180) * Math.PI;
